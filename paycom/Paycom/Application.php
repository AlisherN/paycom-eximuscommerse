...
class Application
{
    ...
    private function CheckPerformTransaction()
    {
        ...
        // validate parameters
        $order->validateTransaction($this->request->params);
    ...
    private function CheckTransaction()
    {
        ...
        $this->response->send([
            'create_time'  => 1000 * Format::datetime2timestamp($found->create_time),
            'perform_time' => 1000 * Format::datetime2timestamp($found->perform_time),
            'cancel_time'  => 1000 * Format::datetime2timestamp($found->cancel_time),
    ...
    private function CreateTransaction()
    {
        ...
        if ($found) {
            ...
            $create_time                        = 1000 * Format::timestamp();
        ...
        switch ($found->state) {
            case Transaction::STATE_CREATED: // handle active transaction
                if ($found->isExpired()) { // if transaction is expired, then cancel it and send error
                    ...
                    $perform_time        = 1000 * Format::timestamp();
